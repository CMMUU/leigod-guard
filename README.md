...
**当前版本 0.17.0：** 支持雷神、外星仔的本机自动暂停，并可分别授权服务器失联保护。外星仔需独立登录、在官方客户端暂停并完成状态校准；2026-09-23 已收到用户“测试可用”的本机反馈，具体场景未逐项确认。服务器外星仔适配仍需真实账号验收。见[外星仔首次配置](#外星仔首次配置试验性)与[接入说明](docs/外星仔接入说明.md)。两款加速器的服务器授权均默认关闭，平台登录不会自动开启。

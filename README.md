...
3. **后台守护计时**：保持工具运行，按下方「自动暂停规则」检查游戏。准备开玩时，可在首页或托盘点击「准备游戏，保护10分钟」，也适用于重新启动游戏。暂停后，打开雷神官方微信小程序，登录同一账号并下拉刷新，核对计时状态。
...
**工具无法识别你在雷神客户端点击“开启加速”的动作。** 准备开玩时可主动使用准备游戏按钮，并把实际游戏进程加入名单。启动检查结束后手动开启加速不会被工具自动感知，但仍可点击准备游戏保护；PUBG 新启动链也能触发有限保护。观察到游戏后，正常退出监控继续工作。如果其他任务本身需要持续加速（如加速下载），不希望被启动或游戏退出检查暂停，请提前关闭自动暂停总开关；关机/注销暂停仍由独立选项控制。本工具不判断任务内容或下载进度。
...
- 账户页与托盘菜单的「立即暂停计时」。
- 分别设置启动等待和退出宽限期；在首页或托盘显式保护首次启动或重新启动游戏。

...
点击托盘图标、托盘菜单「打开面板」，或再次运行桌面/开始菜单快捷方式即可打开界面；手动首次运行也会正常显示。关闭主窗口会隐藏到托盘，托盘「退出」才会结束程序。首次打开面板前的后台告警会写入日志并暂存，主动打开后显示最新提醒。若托盘或静默启动初始化失败，保留主界面作为操作入口。

## 账户剩余时长与自动刷新

首页启动保护卡片同时显示**账户剩余时长**和**启动检查倒计时**：前者来自账户查询，完整显示时、分、秒（例如「8941 时 15 分 01 秒」）；后者表示距离本次启动复查还需等待多久，两者相互独立。
...
| 版本 | 下载后怎样使用 | 适合谁 |
| --- | --- | --- |
| [安装版 EXE](https://github.com/CMMUU/leigod-guard/releases/download/v0.11.4/leigod-guard-v0.11.4-windows-x64-setup.exe) | 双击安装，按需自动补装 WebView2，提供快捷方式和卸载入口 | 普通用户，推荐 |
| [绿色免安装版 ZIP](https://github.com/CMMUU/leigod-guard/releases/download/v0.11.4/leigod-guard-v0.11.4-windows-x64.zip) | 解压到固定且可写的目录，运行 `leigod-guard.exe`；验证码需要已有 WebView2 Runtime | 希望自行管理程序目录的用户 |
...
1. 下载 [Windows x64 安装包](https://github.com/CMMUU/leigod-guard/releases/download/v0.11.4/leigod-guard-v0.11.4-windows-x64-setup.exe)：`leigod-guard-v0.11.4-windows-x64-setup.exe`。页面中的 `Source code` 是源码，不是安装程序。
...
绿色免安装版下载文件为 `leigod-guard-v0.11.4-windows-x64.zip`，请完整解压后运行，不要直接从 ZIP 预览窗口启动。该版本不提供安装和卸载向导；普通用户建议使用上面的安装版。
...
```powershell
Get-FileHash .\leigod-guard-v0.11.4-windows-x64-setup.exe -Algorithm SHA256
```

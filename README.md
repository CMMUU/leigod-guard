...
点击托盘图标、托盘菜单「打开面板」，或再次运行桌面/开始菜单快捷方式即可打开界面；手动首次运行也会正常显示。右上角关闭按钮会隐藏到托盘，托盘「退出」才会结束程序。标题栏支持拖动、双击最大化或还原，以及边缘拖动缩放；最小化按钮会最小化到任务栏。首次打开面板前的后台告警会写入日志并暂存，主动打开后显示最新提醒。若托盘或静默启动初始化失败，保留主界面作为操作入口。

...
- 只读元数据 GET 遇到传输异常或 HTTP 500/502/503/504 时，最多尝试 3 次，间隔 2、4 秒；每次仍保留原超时与凭据边界。权限错误、限频、无效响应及写操作不自动重试。上传结果不确定时仍应通过重跑检查已有附件，不能盲目重复 POST。
...
Server CI 使用一次性 PostgreSQL 与回环模拟邮件服务，覆盖验证码限频、错误次数、过期、重发、并发兑换、普通用户权限、30 天会话及旧密码 API。设备测试覆盖稳定安装身份、凭据轮换、跨账号隔离、撤销后禁止自动恢复、手动配对、解除归属和雷神标识关联清除。

Windows CI 继续使用隔离 Windows 执行实际 DPAPI、原生界面、安装、升级、卸载和自动更新验证。设备文件额外验证身份与预留心跳计数持久化。真实邮件只使用用户明确指定的验收地址，不将验证码或完整邮箱写入文档；模拟邮箱通过不代表真实送达。

后台升级前备份数据库，并保留旧版本目录。应用 `0002_email_devices.sql` 后，旧 0.1 二进制不能直接启动：它缺少已应用的迁移版本，SQLx 会拒绝启动，且旧设备查询无法处理未知游戏状态。应优先修复后向前升级；若需回退业务逻辑，必须先构建并验证包含 0002 迁移、兼容 nullable 游戏状态的新恢复包。不能仅切换旧 release，也不能删除迁移记录或用旧备份覆盖新增用户。此处更正 v0.14.0 标签随包说明中不完整的直接回退步骤。邮件配置必须由服务器受限环境文件提供；`EMAIL_TEST_ENDPOINT` 只能用于回环测试。

## 服务器失联保护（0.15.0）

发版前执行 [REMOTE_PROTECTION.md](REMOTE_PROTECTION.md) 中的服务端、客户端和故障场景验证。生产先备份、安装独立凭据文件及 0003 迁移，再部署能力；每台设备远程保护默认关闭，由用户明确授权。真实雷神账号暂停只能在用户指定的测试窗口进行，未验证应在 Release 说明保留边界。正式资产仍由 GitHub 构建一次，Gitee 复制并逐个下载校验。

全局开关可暂停远程执行，但不能把旧后台二进制直接回退到已应用新迁移的数据库。恢复/密钥轮换和异地备份见运维文档；不在聊天、发布工件或仓库保存原始凭据。


## 游戏启动与重启保护（0.18.0）

新增独立进程观察和有限启动阶段；发布前覆盖 PUBG 现场启动/重启时间线、旧辅助进程残留、同轮子进程接力不续期、进程元数据不可读、PID 重用、快照过期/失败、名单更换、慢登录后新轮次与暂停前复查。雷神和外星仔使用同一观察；首页、名单、平台心跳不能各自推断出互相矛盾的进程状态。心跳中的自动启动阶段映射为未知，不能将自动保护反复写入显式准备期。

原启动补查 180 秒、退出宽限期 90 秒均保留用户配置；新增 `strategy.launch_grace_secs` 默认 600 秒、范围 30–3600 秒，已开始的一轮截止时间不因后续启动器或配置变化而延长。主进程出现后转入正常退出监控；保护耗尽且仍无主进程时，在有效观察下重新经过完整退出确认。手动准备也支持检查结束后启动/重启。原生截图增加启动保护的普通和窄窗口场景。

CI 不操作用户正在游戏的电脑，不触发真实账户暂停。合成时间线、隔离进程及渲染测试不能替代真实 PUBG 启动、对局和退出验收；这部分由用户更新后反馈。服务器失联策略和后台版本不在本次变更范围。

...
0.11.2 起的退出宽限期显示需覆盖：后台尚未再次扫描时仍逐秒更新、跨过截止时间不出现负数或提前报告成功、后续扫描不重置显示起点、策略变化与游戏重开清理旧状态。原生截图新增退出宽限期的正常窗口与窄窗口场景；进程扫描、账户查询和暂停调用仍由原有后台策略驱动。
...
7. 对更新功能，检查启动检查默认关闭、自动来源默认值、旧配置保留、同版国内优先、不同步时选新、并发限时、严格单源、来源失败与无新版区分、固定版本备用下载、缺失资产和哈希不匹配、持续慢速下载与停滞超时；核对安装版和绿色版分别保持发行方式、更新后版本正确且用户配置不变。升级失败或文件被占用时还应检查恢复结果，不能仅凭下载成功认定自动更新通过。
...
0.10.1 起，账户刷新须在独立后台线程进行，并在成功、失败、超时和线程中断时结束等待。离线测试应覆盖重复点击、页面切换、旧结果标注、未知计时状态以及退出登录或切换账户后丢弃迟到响应。连接超时 5 秒、请求超时 12 秒，界面等待上限 15 秒。模拟响应只能验证界面和请求生命周期，不能描述为真实雷神账户验证通过；刷新本身不得调用暂停或恢复接口。

0.11.0 起，还应覆盖首页基础及体验时长合计、数字与字符串字段、零余额、不足一分钟、未知或特殊套餐、查询时间、前台刷新间隔、隐藏窗口不定时查询，以及账号切换后清除旧余额。毛玻璃验证直接使用正式渲染器输出，检查背景模糊、透明图层的颜色混合、文字清晰度、宽窄窗口及 150% DPI；该效果作用于缓存的应用内背景，不涉及桌面采集。真实账户的特殊套餐与接口兼容性仍需独立核对。

对关机、系统崩溃、断网和非公开接口的限制，沿用 README 的说明，不把一次成功测试描述为永久兼容保证。

0.11.1 还需验证活动到期时间恰好等于服务器当前时间时仍显示有效余额、活动进行中不重复合计体验时长，以及启动、托盘打开、进入账户页、快速重复打开、前台定时刷新和查询失败后的间隔控制。账户查询只读取状态，不执行暂停或恢复。真实账户诊断与自动化回归分开记录；不得在源码、截图工件或发布文件中保存实际凭据与账户原始响应。

## 发布标签与 Release

0.12.2 的静默启动回归补充 `SetWindowLongW` 直接写入 `WS_VISIBLE` 的路径：旧版只拦截 `ShowWindow` / `SetWindowPos`，无法阻止 winit 随后的可见属性更新。测试同时验证临时禁止激活的属性不会被覆盖，并在主动打开时恢复。窗口消息处理依据 [WM_STYLECHANGING](https://learn.microsoft.com/windows/win32/winmsg/wm-stylechanging) 与 [WM_WINDOWPOSCHANGING](https://learn.microsoft.com/windows/win32/winmsg/wm-windowposchanging) 的属性修改规则。

CI 与发布工作流增加 `scripts/test-silent-start.ps1`：在独立进程运行 `examples/silent_start_probe.rs`，通过实际 eframe 的首次绘制与持续重绘检查窗口可见性和前台焦点，覆盖静默启动、手动启动、跨线程打开、隐藏与重新打开。测试窗口微小且在屏幕外，不加载用户配置、账户、托盘、自动暂停线程或正式程序的实例锁，30 秒未完成即失败。已有自启项继续使用 `--minimized`；检查不等于真实 Windows 重启验证，不重启用户电脑，也不启动或更新正在使用的正式应用。
...
仓库的 [CI 工作流](https://github.com/CMMUU/leigod-guard/blob/v0.13.0/.github/workflows/ci.yml) 执行自动检查，[发布工作流](https://github.com/CMMUU/leigod-guard/blob/v0.13.0/.github/workflows/release.yml) 根据推送的 `v*` 标签构建安装版、绿色免安装版和校验文件并发布，也可通过 `workflow_dispatch` 指定已有标签。已有同名 Release 时，工作流会拒绝覆盖。
...
- 手动同步默认 `scope=release`，可填写 `tag`（例如 `v0.13.0`）处理指定公开版本，留空选择 GitHub 最新正式版；`scope=all` 单独执行完整历史同步和校验，`scope=refs` 只同步代码和标签。`all`、`refs` 不接受 `tag`。自动发版不再逐次下载全部历史附件，旧版附件异常不会阻塞独立的新版本同步；目标版本仍执行完整的大小与 SHA-256 校验。
...
新增 `scripts/test-session-end.ps1`，仅对隔离测试进程中的离屏 eframe 窗口发送消息，绝不向系统广播或触发真实关机，也不加载用户账号配置。覆盖静默启动、手动启动、静默后主动打开三个入口，以及询问/取消/确认/重复通知。`--legacy-missing-main-handler` 用于复现仅主窗口收到通知时旧路径遗漏暂停的对照失败。单元测试另覆盖共享截止时间、并发接收者、锁繁忙、异常、未登录和关闭策略。

主窗口永久子类处理与独立隐藏窗口共享一次暂停任务，总等待最多4秒，HTTP请求最多3秒；仅收到会话结束确认为真才暂停。应用使用 `SetProcessShutdownParameters(0x3ff, 0)` 尽早接收正常关机通知；失败使用默认顺序，不修改系统超时、不阻止关机。结果记录不包含原始账号凭据。

此修复是本地尽力处理，不是服务器兜底。真实系统关机和真实账户暂停结果尚需核验；断电、强制终止、断网、过期凭据仍不能保证暂停。

## 平台账号登录验证（0.13.0）

平台账号与雷神加速器账号独立。发布检查覆盖登录 Cookie 与 CSRF、身份和有效期、拒绝重定向、网络错误、DPAPI 会话保存/恢复/清除，以及原生登录、登录成功和断网布局。上海 HTTPS 联调使用临时普通账号，验证后清理；不将该结果描述为真实雷神账户计时验证。客户端不会发送设备心跳或执行远程暂停。

...
| 资产 | 用途 |
| --- | --- |
| `leigod-guard-v0.11.4-windows-x64-setup.exe` | 推荐下载；Windows x64 安装程序 |
| `leigod-guard-v0.11.4-windows-x64.zip` | 绿色免安装程序包 |
...
0.11.4 的静默自启回归使用独立、屏幕外且不激活的 Win32 测试窗口：验证 `ShowWindow` 与 `SetWindowPos` 的首次及重复显示被拦截，跨线程主动打开可解除拦截，之后隐藏和再次打开正常。检查已有自启项仍使用 `--minimized`、重复静默启动不唤起已有实例、手动启动及托盘打开正常；保持账户与自动暂停规则的隔离，不通过重启用户电脑验证，不启动用户实际账户的第二个监控实例。

先将通过检查的源码和文档提交到仓库，确认提交已经推送。然后为该提交创建带说明的版本标签，例如：

```powershell
git tag -a v0.11.4 -m "Release v0.11.4"
git push origin v0.11.4
```

仓库的 [CI 工作流](https://github.com/CMMUU/leigod-guard/blob/v0.11.4/.github/workflows/ci.yml) 执行自动检查，[发布工作流](https://github.com/CMMUU/leigod-guard/blob/v0.11.4/.github/workflows/release.yml) 根据推送的 `v*` 标签构建安装版、绿色免安装版和校验文件并发布，也可通过 `workflow_dispatch` 指定已有标签。已有同名 Release 时，工作流会拒绝覆盖。
...
- 手动同步默认 `scope=release`，可填写 `tag`（例如 `v0.11.4`）处理指定公开版本，留空选择 GitHub 最新正式版；`scope=all` 单独执行完整历史同步和校验，`scope=refs` 只同步代码和标签。`all`、`refs` 不接受 `tag`。自动发版不再逐次下载全部历史附件，旧版附件异常不会阻塞独立的新版本同步；目标版本仍执行完整的大小与 SHA-256 校验。

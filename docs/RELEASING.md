...
后台升级前备份数据库，并保留旧版本目录。迁移 `0002_email_devices.sql` 为扩展字段，但游戏状态允许未知值；如需回退旧 0.1 后台，先停止新服务、将 `game_running IS NULL` 的设备状态归一为 false（旧界面无法显示未知），保留迁移版本记录及新增表。优先修复后向前升级，禁止用旧备份直接覆盖新增用户数据。邮件配置必须由服务器受限环境文件提供；`EMAIL_TEST_ENDPOINT` 只能用于回环测试。

...
适用：客户端 0.17.0、后台 0.4.0。完整分析及最终验收存于 [项目文档仓库](https://github.com/CMMUU/project-docs/tree/main/leigod-guard)。历史规划见 [REMOTE_GUARD_PLAN.md](REMOTE_GUARD_PLAN.md)。

## 授权与工作流程

平台登录/设备配对不自动开启远程保护。在客户端勾选凭据上传授权并开启后，上海服务器向对应加速器查询账号标识；雷神（当前接口为 `nn_number`，兼容缺少该字段的历史 `user_id/id` 响应）；不能验证账号标识、凭据失效、或该账号归属其他平台用户时拒绝授权。仅保存 AES-256-GCM 密文，随机 96 位 nonce，关联账号摘要作为认证附加数据。管理员接口不返回 token 或密文；不收集密码/密码哈希。请求体上限 8 KiB，token 上限 4096 字节，provider 响应上限 64 KiB。

```mermaid
flowchart TD
    A["客户端明确授权并上传当前 token"] --> B["服务器查询官方账号身份并验证归属"]
    B --> C["加密凭据和设备授权入库"]
    ...
    F --> G["领取 90 秒租约并提交事务"]
    G --> H["复核授权、心跳、服务状态和租约"]
    H --> I["查询对应加速器状态；必要时发送暂停"]
    I --> J["再次查询确认或有限重试"]
```

15 秒心跳；45 秒表示连接等待；120 秒为默认失联阈值；最多 600 秒准备期。按服务器接收时间计算。同账号任一授权设备新鲜心跳（游戏状态为真、假或未知均相同）都会阻止失联暂停；旧的“游戏运行中”不会永久阻止离线后的暂停。本地在线无游戏策略保持独立。后台 0.5.0 新增的网吧模式开启时，账号的云端执行改用下面的连续计时时长规则。

## 网吧模式（后台 0.5.0）

用户登录后台，进入“网吧模式”，选择已授权的雷神或外星仔账号，点击“设置网吧模式”，明确开启并保存。默认连续未暂停上限为 **24 小时**，可设置 **1–168 整数小时**；升级不自动替任何账号开启。首次绑定/凭据失效仍需在已有客户端授权，网吧电脑不必安装守护。

- 云端约每 60 秒查询官方计时状态，与暂停任务共用 2 个并发名额。负载或故障会延后查询；页面显示最近确认时间。关闭网页、电脑离线不撤销网吧策略。
- 从服务器首次确认正在计时开始计算；心跳和重复查询不重置起点。确认暂停后清空本轮，再次观察到计时才创建新轮次。不能还原启用前的计时时长，也不能识别两次轮询之间短暂的暂停/恢复。
- 修改小时数保留当前轮次起点，缩短到已经过去的时长可能立即执行。开启/关闭后的新轮次重新观察，不把旧记录当作新的使用起点。
- 设置和起点保存在 PostgreSQL，短暂服务重启不丢失。连续超过 5 分钟没有成功确认官方状态，恢复时重新开始观察计时，避免把未知期间误算成连续使用；因此不保证故障期间也恰好 24 小时暂停。
- 到期前要求 150 秒内的新鲜“正在计时”观察，执行时再查询，必要时暂停并查询确认。凭据失效显示重新授权；状态未知不盲停；每轮仍使用最多 3 次/5 分钟的有限执行窗口。
- 网吧模式优先于同账号云端失联规则，家中设备失联不会提前触发 120 秒暂停，设备正常心跳也不取消网吧到期任务。时长上限作用于整个加速器账号，到期时即使仍在游戏也可能暂停；已安装客户端的本机规则不受云端模式控制。
- 关闭网吧模式取消未发送的旧任务；原设备失联保护需收到新的有效心跳后恢复，避免立即执行历史失联。已发送的官方请求无法保证撤回。
- 这是独立的账号授权：关闭设备保护、设备撤销或退出客户端不自动关闭网吧模式；在网页单独关闭。只要仍有有效设备或网吧授权就保留加密凭据，最后一个授权关闭后删除密文。平台用户被停用会同时关闭云端策略并删除其可用凭据。

后台 0.5.0 需要迁移 `0005_cafe_mode.sql`。沿用已有服务、数据库与加密密钥，不增加 Redis、外部定时器或浏览器常驻依赖。升级前备份数据库和密钥；迁移后不可直接运行缺少该迁移的旧二进制。
...
同一物理设备可分别授权雷神和外星仔，按 `(device_id, provider)` 存储授权和独立版本号。账号身份摘要使用不同命名空间，任务仍按官方账号聚合。手动关闭只影响所选加速器；设备撤销、重新绑定和用户停用原子撤销两者。

外星仔使用固定 `https://api.et-api.com`：GET `/account/v1/my_profile` 取得官方 `user_id`，POST `/v2/account/remain/duration` 查询 PC 状态，POST `/v2/account/update/pause/state` 请求校准的暂停值。身份字段的公开协议线索与验证限制见[外星仔接入说明](外星仔接入说明.md)。不使用客户端传来的手机号、账号 ID 或 token 哈希合并设备。

首次授权必须查询到与用户校准值一致的已暂停状态。后续同账号刷新凭据允许计时运行，但不能改写已验证的暂停值。未知状态不发送写请求。令牌、随机设备 ID、校准值作为整体加密；不收集外星仔密码。

客户端使用独立 `/api/device/remote/etalien` 路径，并验证响应中的 provider，避免旧服务忽略参数后误操作雷神授权。旧客户端心跳缺少 `etalien_revision` 时撤销外星仔授权，避免无人维护的旧授权把在线设备当作失联。
...
0.4.0 增加 `0004_provider_grants.sql`，不修改历史迁移。部署前必须完成数据库备份。
...
- 不能把旧 0.3.x 及更早二进制直接切回来：SQLx 会因已应用 0004 而拒绝启动。优先关闭执行能力并向前修复；恢复包必须包含全部已应用迁移且兼容新 schema。不得删除 migration 记录或用旧备份覆盖新增用户。
- `LEIGOD_TEST_ORIGIN` / `ETALIEN_TEST_ORIGIN` 仅允许在 `PUBLIC_ORIGIN=http://127.0.0.1:<port>` 时指向回环 HTTP 根路径，生产 HTTPS 配置拒绝测试端点。
...
已收到用户对外星仔本机可用的反馈，不等于所有场景均通过。双加速器服务端验证使用隔离 PostgreSQL 与官方协议模拟器；真实外星仔服务器授权、上海出口限制及实际失联暂停需要空闲账号验收。不操作正在游戏的设备。
